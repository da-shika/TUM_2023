...
inline cc::Points2d changeRefFrame(
  const cc::Points2d& points_child,
  const CartesianPosition& X_parent_child)
{
  return (X_parent_child.angular().toRotationMatrix().topLeftCorner(2,2)*points_child).colwise() +
    X_parent_child.linear().head(2);
}
...
inline cc::Points2d changeRefFrame(
  ...
  const HomogeneousTransformation& T_parent_child)
{
  return (T_parent_child.rotation().topLeftCorner(2,2)*points_child).colwise() + 
    T_parent_child.position().head(2);
}
