...
namespace cc{
  ...
  */
  ...
  {
    const Derived1& Q_ = Q.derived();
    Eigen::Matrix<Scalar,3,4> H;
    H << 
      Q_.w(), -Q_.z(), Q_.y(), -Q_.x(),
      Q_.z(), Q_.w(), -Q_.x(), -Q_.y(),
      -Q_.y(), Q_.x(), Q_.w(), Q_.z();
    return Scalar(2)*H*QP.derived().coeffs();
  }
   ...
   */
  template <typename Derived1, typename Derived2>
  inline AngularPosition integrateVelocityBody(
   ...
   */
  template <typename Derived1, typename Derived2>
  inline AngularPosition integrateVelocityWorld(
    const control_core::QuaternionBase<Derived1>& Q_b_w,
  ...
  /**
   * @brief compute the average Quaternion
   * 
   * Matirx A \in 4xn holds n weighted Quaternions in its colums.
   * The i-th column coresponds to weight[i]*Q[i].
   * 
   * The average quaternion is the larges eigen vector of A*A.transpose()
   * See: http://www.acsu.buffalo.edu/%7Ejohnc/ave_quat07.pdf
   * 
   * @param A 
   * @return cc::AngularPosition 
   */
  inline cc::AngularPosition average(
    const Eigen::Matrix<cc::Scalar,4,-1>& A)
  {
    cc::AngularPosition avg;
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix4d> solver;
    solver.compute(A*A.transpose());
    avg.coeffs() = solver.eigenvectors().col(3);
    return avg;
  }
   ...
   * @brief compute the average Quaternion
   * 
   * Vector weights holds the weights of each Quaternion 
   * Note: sum(weights)=1
   * 
   * Vector Qs holds the Quaternions
   * 
   * @param A 
   * @return cc::AngularPosition 
   */
  inline cc::AngularPosition average(
    ...
    const std::vector<cc::AngularPosition>& Qs)
  {
    Eigen::MatrixXd A(4, Qs.size());
    for(size_t i = 0; i < Qs.size(); ++i)
      A.col(i) = weights[i]*Qs[i].coeffs();
    return average(A);
  }
