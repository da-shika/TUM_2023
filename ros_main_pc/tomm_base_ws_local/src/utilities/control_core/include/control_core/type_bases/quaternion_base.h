...
#include <control_core/math/euler.h>
...
public:
  ...
  */
  ...
  /*!
   * \brief Conversion to euler anlges by eigen
   */
  ...
  /*!
   * \brief Conversion to euler anlges yaw, pitch roll.
   */
  control_core::Vector3<Scalar> eulerYPR() const
  {
    return cc::eulerYPR(Base::derived().toRotationMatrix());
  }

  /**
   * @brief return the xy roation matrix (planar rotation)
   * 
   * @return Eigen::Matrix<Scalar,2,2> 
   */
  Eigen::Matrix<Scalar,2,2> toRotationMatrix2d() const
  {
    return Base::derived().toRotationMatrix().topLeftCorner(2,2);
  }
