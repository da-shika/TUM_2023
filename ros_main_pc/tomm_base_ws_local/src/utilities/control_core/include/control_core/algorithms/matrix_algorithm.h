...
public:
  ...
  typedef std::unique_ptr<IAlgorithm> IAlgorithmPtr;

protected:
  std::vector<IAlgorithmPtr> algos_;      //!< vector of algos_.
  Derived y_;                             //!< output
...
public:
   ...
   * For now the size cannot be changed after construction.
   */
  MatrixAlgorithm(const IAlgorithm& algo, Index size)
  {
    bool is_valid = ((Rows == Eigen::Dynamic) || (Cols == Eigen::Dynamic));
    ROS_ASSERT_MSG(is_valid,
                   "Invalid constructor. "
    ...
    size_ = rows_*cols_;
    y_.resize(rows_, cols_);

    setup(algo);
  }

  MatrixAlgorithm(const IAlgorithm& algo, Index rows, Index cols)
  {
  ...
  virtual const Derived& value() const
  {
    return y_;
  }
