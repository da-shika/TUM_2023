...
public:
  ...
  */
  ...
  {
    return cc::uniformDistributedQuaternion<Scalar>(generator);
  }
...
public:
  ...
  /*!
   * \brief Assignment from Euler YPR
   *
   * Internally the coefficients are stored in the following order:
   * [x, y, z, w]
   */
  AngularPosition(const Scalar& yaw, const Scalar& pitch, const Scalar& roll) : 
    Base(
