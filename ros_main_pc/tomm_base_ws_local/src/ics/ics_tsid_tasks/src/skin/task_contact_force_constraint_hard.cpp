...
namespace tsid
{
  namespace tasks
  {
    ...
    TaskContactForceConstraintHard::TaskContactForceConstraintHard(const std::string & name,
                                           cc::Scalar dt,
    ...
    bool TaskContactForceConstraintHard::connect(ros::NodeHandle& nh)
    {
      //////////////////////////////////////////////////////////////////////////
      // load params
      //////////////////////////////////////////////////////////////////////////
      ...
      {
        ROS_ERROR("TaskContactForceConstraintHard:init(): Failed to init parameters");
        return false;
      }
      // setup parameters server
      reconfig_srv_ = std::make_unique<Server>(params_.privateNamespace());
      reconfig_srv_->setCallback(boost::bind(&TaskContactForceConstraintHard::reconfigureRequest, this, _1, _2));
      ...
      {
        ROS_ERROR("TaskContactForceConstraintHard::connect: Topic '%s' not yet published", contact_topic_.c_str());
        //return false;
      }
      wbc_contacts_sub_ = nh.subscribe(contact_topic_, 1, &TaskContactForceConstraintHard::callback, this);
      ...
      ROS_INFO_STREAM("TaskContactForceConstraintHard::connect(): max contacts=" << num_max_ieq_);
      return true;
    }

    void TaskContactForceConstraintHard::setMask(ConstRefVector m)
    {
      assert(m.size()==m_robot.na());
      m_mask = m;
    }

    double TaskContactForceConstraintHard::minimumDistance() const
    {
      return min_distance_;
    }

    int TaskContactForceConstraintHard::numActiveConstraints() const
    {
      return num_active_ieq_;
    }

    const ConstraintBase & TaskContactForceConstraintHard::compute(const double t,
                                                    ConstRefVector q,
                                                    ConstRefVector v,
                                                    Data & data)
    {
      ...
      for(size_t i = 0; i < std::min(contacts_.size(), num_max_ieq_); ++i)
      {
        ...
        double prox = contact.proximity().W().z();
        double fmod = f + prox;

        ROS_WARN_STREAM_THROTTLE(0.5, "prox=" << prox);
        ROS_WARN_STREAM_THROTTLE(0.5, "f=" << f);
        ROS_WARN_STREAM_THROTTLE(0.5, "fmod=" << fmod);

        if(fmod > params_.f_min)
        {
          ...
          // acceleration limit
          cc::Scalar acc_limit = 1./params_.mass*(params_.gain*(params_.f_max - fmod) - params_.damp*dP);
          ...
          else
          {
            ...
            bieq.tail(4).setConstant(-1.0); // constraint_.upperBound()[k]    // no need for real velocity!
            num_violated_ieq_ += 4;
          }
      ...
      if(num_violated_ieq_ > 0)
      {
        ...
        else
        {
          ...
          for(size_t l = 0; l < qp_.getActiveSetSize(); ++l)
          {
            idx = active[l];
            frame_id = violated_frame_ids_[std::floor(idx / 4)];
    ...
    void TaskContactForceConstraintHard::reconfigureRequest(Config &config, uint32_t level)
    {
      params_.updateConfig(config, level);
    }

    void TaskContactForceConstraintHard::callback(const control_core_msgs::SkinPatchesConstPtr& msg)
    {
    ...
    std::shared_ptr<TaskBase> TaskContactForceConstraintHard::Load(
      ...
      bool verbose)
    {
      ...
      if (!parameter.load())
      {
        ROS_ERROR("load_contact_force_constraint_task: Failed to load parameters of '%s'", name.c_str());
        return nullptr;
      }

      auto task = std::make_shared<TaskContactForceConstraintHard>(
