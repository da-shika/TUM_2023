...
class SARNN(nn.Module):
    ...
    def forward(self, xi, xj, state=[None,None,None]):
        """
        ...
        # concat
        left_hid = torch.cat([xj[0], left_enc_pts], -1)
        right_hid = torch.cat([xj[1], right_enc_pts], -1)

        # rnn
        [left_hid, right_hid], state = self.rec(left_hid, right_hid, state)  # LSTM forward pass

        # vec predict
        y_left_joint = self.decoder_left_vec(left_hid[0])  # Decode vec prediction
        y_right_joint = self.decoder_right_vec(right_hid[0])  # Decode vec prediction
