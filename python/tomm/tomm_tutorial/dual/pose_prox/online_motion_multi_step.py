...
try:
    from libs.model_multi_step import SARNN
except:
    sys.path.append("./libs/")
    from model_multi_step import SARNN
...
class OnlineSARNN:
    def __init__(self, data_dir, model_name, inference_rate=10, max_episode_steps=270):
        ...
        self.left_target_traj_msg = Path()
        self.right_target_traj_msg = Path()
        self.reset()
        ...
        self.target_left_traj_pub = rospy.Publisher("/target/left_traj", Path, queue_size=1)
        self.target_right_traj_pub = rospy.Publisher("/target/right_traj", Path, queue_size=1)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def run(self):
        ...
        while not rospy.is_shutdown() and self.steps < self.max_episode_steps:
            ...
            y_left_image, y_right_image, y_left_poses, y_right_poses, y_left_proximitys, y_right_proximitys, \
                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state = self.inference(state)

            # send to robot
            if self.steps > self.warmup_steps:
                self.publish(y_left_poses[0], self.left_target_pose_msg, self.target_left_pos_pub)
                self.publish(y_right_poses[0], self.right_target_pose_msg, self.target_right_pos_pub)
                self.publishTrajectory(y_left_poses, self.left_target_traj_msg, self.target_left_traj_pub)
                self.publishTrajectory(y_right_poses, self.right_target_traj_msg, self.target_right_traj_pub)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def inference(self, state):
        ...
        with torch.no_grad():
            y_image, y_poses, y_proximitys, enc_pts, dec_pts, state = \
                self.model(x_image, x_pose, x_proximity, state, 20)
            
        # denormalization
        y_left_image = self.denorm_image(y_image[0][0])
        y_right_image = self.denorm_image(y_image[1][0])

        y_left_poses, y_right_poses = [], []
        y_left_proximitys, y_right_proximitys = [], []
        for i in range(len(y_poses[0])):

          y_left_poses.append(self.denorm_signal(y_poses[0][i][0], self.left_pose_bounds))
          y_right_poses.append(self.denorm_signal(y_poses[1][i][0], self.right_pose_bounds))

          y_left_proximitys.append(self.denorm_signal(y_proximitys[0][i][0], self.left_proximity_bounds))
          y_right_proximitys.append(self.denorm_signal(y_proximitys[1][i][0], self.right_proximity_bounds))
        ...
        return y_left_image, y_right_image, y_left_poses, y_right_poses, y_left_proximitys, y_right_proximitys, \
    ...
    def publishTrajectory(self, y_poses, path_msg, target_pub):
        path_msg.poses = []
        for i in range(len(y_poses)):
          pose_list = y_poses[i].tolist()
          ...
          target_msg.header.stamp = rospy.Time.now()
          target_msg.header.frame_id = 'tomm/base_link'
          path_msg.poses.append(target_msg)
        
        path_msg.header.stamp = rospy.Time.now()
        path_msg.header.frame_id = 'tomm/base_link'
        target_pub.publish(path_msg)
