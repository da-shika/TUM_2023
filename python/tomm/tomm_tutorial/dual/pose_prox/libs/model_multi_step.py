...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        vec_dim = pose_dim + proximity_dim
    ...
    def forward(self, xi, xpos, xp, state=[None,None,None], pred_steps=1):
        """
        ...
        # concat
        xv_l = torch.cat((xpos[0], xp[0]), -1)
        xv_r = torch.cat((xpos[1], xp[1]), -1)
        left_hid = torch.cat([xv_l, left_enc_pts], -1)
        right_hid = torch.cat([xv_r, right_enc_pts], -1)
        ...
        y_left_poses, y_left_proximitys = [], []
        y_right_poses, y_right_proximitys = [], []
        ...
        for i in range(pred_steps):
          ...
          y_left_vec = self.decoder_left_vec(left_hid[0])  # Decode vec prediction
          y_right_vec = self.decoder_right_vec(right_hid[0])  # Decode vec prediction

          split_points = [self.pose_dim, self.proximity_dim]
          y_left_pose, y_left_proximity = torch.split(y_left_vec, split_points, dim=1)
          y_right_pose, y_right_proximity = torch.split(y_right_vec, split_points, dim=1)
          ...
          # remember sequence
          y_left_poses.append(y_left_pose)
          y_left_proximitys.append(y_left_proximity)

          y_right_poses.append(y_right_pose)
          y_right_proximitys.append(y_right_proximity)

          states.append(state)
          left_hids.append(left_hid)
          right_hids.append(right_hid)

          left_hid = torch.cat([y_left_vec, left_dec_pts], -1)
          right_hid = torch.cat([y_right_vec, right_dec_pts], -1)

        y_left_image = self.decoder(left_dec_pts, left_im_hid)
        y_right_image = self.decoder(right_dec_pts, right_im_hid)
        
        return [y_left_image, y_right_image], [y_left_poses, y_right_poses], \
                [y_left_proximitys, y_right_proximitys], \
                [left_enc_pts, right_enc_pts], [left_dec_pts, right_dec_pts], states[0]
