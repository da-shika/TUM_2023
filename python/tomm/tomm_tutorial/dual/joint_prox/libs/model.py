...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        vec_dim = joint_dim + proximity_dim
    ...
    def forward(self, xi, xj, xp, state=[None,None,None]):
        """
        ...
        # concat
        xv_l = torch.cat((xj[0], xp[0]), -1)
        xv_r = torch.cat((xj[1], xp[1]), -1)
        ...
        split_points = [self.joint_dim, self.proximity_dim]
        y_left_joint, y_left_proximity = torch.split(y_left_vec, split_points, dim=1)
        y_right_joint, y_right_proximity = torch.split(y_right_vec, split_points, dim=1)
        ...
        return [y_left_image, y_right_image], [y_left_joint, y_right_joint],\
