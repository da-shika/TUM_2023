...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def run(self):
        ...
        while not rospy.is_shutdown() and self.steps < self.max_episode_steps:
            y_left_image, y_right_image, y_left_joint, y_right_joint, y_left_proximity, y_right_proximity, \
                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, _ = self.inference(state)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def inference(self, state):
        ...
        with torch.no_grad():
            y_image, y_joint, y_proximity, enc_pts, dec_pts, state = self.model(x_image, x_joint, x_proximity, state)
        ...
        return y_left_image, y_right_image, y_left_joint, y_right_joint, y_left_proximity, y_right_proximity, \
                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state


    def publish(self, y_left_joint, y_right_joint):
        y_joint = np.concatenate((y_left_joint, y_right_joint), axis=0)
        self.target_joint_msg.position = y_joint.tolist()
