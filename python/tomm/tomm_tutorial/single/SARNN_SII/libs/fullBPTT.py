...
class fullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            """
            pts = torch.stack(dec_pts_list)
            pts = pts.reshape(list(pts.shape[:2]) + [-1, 2])
            import ipdb; ipdb.set_trace()
            pts_norm = torch.sqrt(torch.sum(torch.pow(torch.diff(pts), 2), dim=(-2, -1)))
            pts_mean = torch.mean(pts_norm, -1)
            pts_mean = pts_mean.repeat((pts.shape[1], 1)).T
            pts_norm_loss = nn.MSELoss()(pts_norm, pts_mean)
            """

            zero_state = torch.zeros_like(state[0], device=self.device, requires_grad=False)
            state_loss = nn.MSELoss()( zero_state, state[0] ) +\
                         nn.MSELoss()( zero_state, state[1] )
            ...
            loss = img_loss + joint_loss + pt_loss + state_loss * self.loss_weights[3]
