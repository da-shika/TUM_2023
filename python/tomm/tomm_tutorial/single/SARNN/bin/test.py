...
# Inference
img_size = 64
image_list, joint_list, pose_list, force_list, proximity_list = [], [], [], [], []
...
for loop_ct in range(nloop):
    ...
    # predict rnn
    y_image, y_joint, y_pose, y_force, y_proximity, ect_pts, dec_pts, state = model(img_t, joint_t, pose_t, force_t, proximity_t, state)
...
def anim_update(i):
    ...
    ax[0,3].tick_params(axis="both", which="both", left=False, right=False, bottom=False, top=False, \
                        labelbottom=False, labelleft=False, labelright=False, labeltop=False)
    ax[0,3].set_facecolor("none")
