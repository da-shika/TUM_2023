...
class SARNN(nn.Module):
    ...
    def forward(self, xi, xj, xpos, xf, xp, state=None):
        """
        ...
        xv = torch.cat((xj, xpos, xf, xp), -1)
        hid = torch.cat([enc_pts, xv], -1)

        rnn_hid = self.rec(hid, state)  # LSTM forward pass
        y_vec = self.decoder_joint(rnn_hid[0])  # Decode joint prediction
        split_points = [self.joint_dim, self.pose_dim, self.force_dim, self.proximity_dim]
        y_joint, y_pose, y_force, y_proximity = torch.split(y_vec, split_points, dim=1)
        ...
        return y_image, y_joint, y_pose, y_force, y_proximity, enc_pts, dec_pts, rnn_hid
