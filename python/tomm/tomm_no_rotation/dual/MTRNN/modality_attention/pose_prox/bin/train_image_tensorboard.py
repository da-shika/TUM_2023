...
try:
    ...
    from python.TUM.tomm_no_rotate.MTRNN_dual.pose_prox.libs.model_bk import MTRNN
except:
...
parser.add_argument("--fast_dim", type=int, default=20)
parser.add_argument("--slow_dim", type=int, default=5)
parser.add_argument("--union_dim", type=int, default=4)
parser.add_argument("--fast_tau", type=int, default=3)
parser.add_argument("--slow_tau", type=int, default=6)
parser.add_argument("--union_tau", type=int, default=12)
...
# load dataset
detaset_folder = "/home/shikada/dataset/TUM/tomm_no_rotate/dual/all/remove_pose_noise/"
...
# define model
model = MTRNN(
    fast_dim=args.fast_dim, slow_dim=args.slow_dim, union_dim=args.union_dim,
    fast_tau=args.fast_tau, slow_tau=args.slow_tau, union_tau=args.union_tau, 
    pose_dim=3,
    proximity_dim=1,
    k_dim=args.k_dim,
        ...
        if epochs_counter % 50 == 0:
            with torch.no_grad():
                ...
                for step in range(20):
                    image = [test_dataset[0][0][0][step].unsqueeze(0), test_dataset[0][0][1][step].unsqueeze(0)]
                    pose = [test_dataset[0][0][2][step].unsqueeze(0), test_dataset[0][0][3][step].unsqueeze(0)]
                    prox = [test_dataset[0][0][4][step].unsqueeze(0), test_dataset[0][0][5][step].unsqueeze(0)]
                    y_image, _, _, enc_pts, dec_pts, state = model(image, pose, prox, state)
                ...
                for i in range(args.k_dim):
                    cv2.circle(left_image, tuple(left_dec_pts[i].astype(np.int32)), 2, (0,0,255), 5)      # RED
                    cv2.circle(left_image, tuple(left_enc_pts[i].astype(np.int32)), 2, (0,255,0), 5)      # GREEN
                    cv2.circle(right_image, tuple(right_dec_pts[i].astype(np.int32)), 2, (0,0,255), 5)    # RED
                    cv2.circle(right_image, tuple(right_enc_pts[i].astype(np.int32)), 2, (0,255,0), 5)    # GREEN

                writer.add_image("Left Image/step 20",  left_image.transpose(2,0,1).astype(np.uint8), global_step=epoch)
                writer.add_image("Right Image/step 20",  right_image.transpose(2,0,1).astype(np.uint8), global_step=epoch)
