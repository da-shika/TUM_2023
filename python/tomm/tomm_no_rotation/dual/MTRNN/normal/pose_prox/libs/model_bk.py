...
class MTRNNCell(nn.Module):
    ...
    def __init__(self,
                 input_dim_enc_pt, input_dim_pose, input_dim_prox, 
        ...
        self.input_dim_enc_pt = input_dim_enc_pt
        ...
        def _make_one_side_MTRNN():
            # Input Layers
            pose2f = nn.Linear(self.input_dim_pose, fast_dim, bias=use_bias)
            enc_pt2f = nn.Linear(self.input_dim_enc_pt, fast_dim, bias=use_bias)
            prox2f = nn.Linear(self.input_dim_prox, fast_dim, bias=use_bias)

            # Fast context layer
            f2f = nn.Linear(fast_dim, fast_dim, bias=False)
            f2s = nn.Linear(fast_dim, slow_dim, bias=use_bias)

            # Slow context layer
            s2s = nn.Linear(slow_dim, slow_dim, bias=False)
            s2f = nn.Linear(slow_dim, fast_dim, bias=use_bias)
            s2u = nn.Linear(slow_dim, union_dim, bias=use_bias)

            # Union context layer
            u2s = nn.Linear(union_dim, slow_dim, bias=use_bias)

            return pose2f, enc_pt2f, prox2f, f2f, f2s, s2s, s2f, s2u, u2s

        self.left_pose2f, self.left_enc_pt2f, self.left_prox2f, self.left_f2f, self.left_f2s, \
            self.left_s2s, self.left_s2f, self.left_s2u, self.left_u2s = _make_one_side_MTRNN()
        self.right_pose2f, self.right_enc_pt2f, self.right_prox2f, self.right_f2f, self.right_f2s, \
            self.right_s2s, self.right_s2f, self.right_s2u, self.right_u2s = _make_one_side_MTRNN()
        
        # Union context layer
        self.u2u = nn.Linear(union_dim, union_dim, bias=False)


    def forward(self, xl_pose, xl_enc, xl_prox,\
                xr_pose, xr_enc, xr_prox, state=None, pb=None):
        ...
        if state is not None:
            prev_left_h_fast, prev_left_h_slow, prev_left_u_fast, prev_left_u_slow, \
            prev_right_h_fast, prev_right_h_slow, prev_right_u_fast, prev_right_u_slow, prev_h_union, prev_u_union = state
        else:
            device = xl_pose.device
            prev_left_h_fast = torch.zeros(batch_size, self.fast_dim).to(device)
            prev_left_h_slow = torch.zeros(batch_size, self.slow_dim).to(device)
            prev_right_h_fast = torch.zeros(batch_size, self.fast_dim).to(device)
            prev_right_h_slow = torch.zeros(batch_size, self.slow_dim).to(device)
            prev_h_union = torch.zeros(batch_size, self.union_dim).to(device)

            prev_left_u_fast = torch.zeros(batch_size, self.fast_dim).to(device)
            prev_left_u_slow = torch.zeros(batch_size, self.slow_dim).to(device)
            prev_right_u_fast = torch.zeros(batch_size, self.fast_dim).to(device)
            prev_right_u_slow = torch.zeros(batch_size, self.slow_dim).to(device)
            prev_u_union = torch.zeros(batch_size, self.union_dim).to(device)

        # left MTRNN
        new_left_u_fast = (1.0 - 1.0/self.fast_tau) * prev_left_u_fast + 1.0/self.fast_tau * (
            self.left_pose2f(xl_pose) + self.left_enc_pt2f(xl_enc) + self.left_prox2f(xl_prox) +
            self.left_f2f(prev_left_h_fast) + self.left_s2f(prev_left_h_slow))

        _input_slow = self.left_f2s(prev_left_h_fast) + self.left_s2s(prev_left_h_slow) + self.left_u2s(prev_h_union)
        if pb is not None:
            _input_slow += pb

        new_left_u_slow = (1.0 - 1.0/self.slow_tau) * prev_left_u_slow + 1.0 / self.slow_tau * _input_slow

        new_left_h_fast = self.activation(new_left_u_fast)
        new_left_h_slow = self.activation(new_left_u_slow)

        # right MTRNN
        new_right_u_fast = (1.0 - 1.0/self.fast_tau) * prev_right_u_fast + 1.0/self.fast_tau * (
            self.right_pose2f(xr_pose) + self.right_enc_pt2f(xr_enc) + self.right_prox2f(xr_prox) +
            self.right_f2f(prev_right_h_fast) + self.right_s2f(prev_right_h_slow))

        _input_slow = self.right_f2s(prev_right_h_fast) + self.right_s2s(prev_right_h_slow) + self.right_u2s(prev_h_union)
        if pb is not None:
            _input_slow += pb

        new_right_u_slow = (1.0 - 1.0/self.slow_tau) * prev_right_u_slow + 1.0/self.slow_tau * _input_slow

        new_right_h_fast = self.activation(new_right_u_fast)
        new_right_h_slow = self.activation(new_right_u_slow)
        ...
        return [new_left_h_fast, new_left_h_slow, new_left_u_fast, new_left_u_slow], \
                [new_right_h_fast, new_right_h_slow, new_right_u_fast, new_right_u_slow], \
                [new_left_h_fast, new_left_h_slow, new_left_u_fast, new_left_u_slow, \
                new_right_h_fast, new_right_h_slow, new_right_u_fast, new_right_u_slow, new_h_union, new_u_union]
