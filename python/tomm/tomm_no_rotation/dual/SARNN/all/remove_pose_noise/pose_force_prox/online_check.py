...
class OnlineSARNN:
    def __init__(self, data_dir, model_name, inference_rate=10, max_episode_steps=270):
        ...
        # load bounds
        data_dir = "/home/genki/ros/workspaces/tomm_base_ws_local/bags/pick_no_rotate/data/dual/all/remove_pose_noise" + "/"
        ...
        self.left_force = npz["left_forces"]
        self.right_force = npz["right_forces"]
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def inference(self, state, i):
        ...
        x_force = [self.process_signal(self.left_force[i], self.left_force_bounds),
                   self.process_signal(self.right_force[i], self.right_force_bounds)]
        x_proximity = [self.process_prox(self.left_proximity[i], self.left_proximity_bounds),
                        self.process_prox(self.right_proximity[i], self.right_proximity_bounds)]
        
        with torch.no_grad():
            y_image, y_pose, y_force, y_proximity, enc_pts, dec_pts, state = \
                self.model(x_image, x_pose, x_force, x_proximity, state)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    def process_prox(self, prox, bounds):
        _prox_remove_channel = [element for idx,element in enumerate(prox) if idx not in [3,4,8,9]]
        _prox_max = max(_prox_remove_channel)
        _prox = normalization(_prox_max, bounds, self.minmax)
        _prox = np.expand_dims(_prox, axis=0)
        return torch.from_numpy(_prox).float()
