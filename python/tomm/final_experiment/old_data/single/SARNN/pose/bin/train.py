...
# load dataset
detaset_folder = "/home/shikada/dataset/TUM/re_no_rotate/single/"
...
# train data
image, left_pose = load_data(train='train', data_type='left')
_, right_pose = load_data(train='train', data_type='right')
pose = np.concatenate((left_pose, right_pose), axis=2)
train_dataset = MultimodalDataset(image, pose, stdev=stdev)
...
pose = np.concatenate((left_pose, right_pose), axis=2)
test_dataset = MultimodalDataset(image, pose, stdev=None)
