...
class fullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        total_loss = 0.0
        for n_batch, ((x_img, x_pose), (y_img, y_pose)) in enumerate(data):
            ...
            T = x_img.shape[1]
            for t in range(T - 1):
                _yi_hat, _ypose_hat, enc_ij, dec_ij, state = self.model(x_img[:, t], x_pose[:, t], state)
            ...
            pt_loss = nn.MSELoss()(torch.stack(dec_pts_list[:-1]), torch.stack(enc_pts_list[1:])) * self.image_scheduler(self.loss_weights[2])

            loss = img_loss + pose_loss + pt_loss
