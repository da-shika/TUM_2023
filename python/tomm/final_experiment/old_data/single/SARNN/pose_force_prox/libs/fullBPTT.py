...
class fullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        total_loss = 0.0
        for n_batch, ((x_img, x_pose, x_force, x_prox), (y_img, y_pose, y_force, y_prox)) in enumerate(data):
            ...
            state = None
            yi_list, ypose_list, yforce_list, yprox_list = [], [], [], []
            dec_pts_list, enc_pts_list = [], []
            T = x_img.shape[1]
            for t in range(T - 1):
                _yi_hat, _ypose_hat, _yf_hat, _yprox_hat, enc_ij, dec_ij, state = self.model(x_img[:, t], x_pose[:, t], x_force[:, t], x_prox[:, t], state)
                yi_list.append(_yi_hat)
                ypose_list.append(_ypose_hat)
                yforce_list.append(_yf_hat)
            ...
            yforce_hat = torch.permute(torch.stack(yforce_list), (1, 0, 2))
            ...
            force_loss = nn.MSELoss()(yforce_hat, y_force[:, 1:]) * self.loss_weights[3]
            prox_loss = nn.MSELoss()(yprox_hat, y_prox[:, 1:]) * self.loss_weights[3]

            loss = img_loss + pose_loss + pt_loss + force_loss + prox_loss
