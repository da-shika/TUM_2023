...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        rec_in = self.pose_dim + self.force_dim + self.prox_dim + self.k_dim*2
        self.rec = nn.LSTMCell(rec_in, rec_dim)  # LSTM cell

        # Decoder
        self.decoder_pose = nn.Sequential(nn.Linear(rec_dim, self.pose_dim), activation)  # Linear layer and activation
        self.decoder_force = nn.Sequential(nn.Linear(rec_dim, self.force_dim), activation)  # Linear layer and activation
        self.decoder_prox = nn.Sequential(nn.Linear(rec_dim, self.prox_dim), activation)
        self.decoder_point = nn.Sequential(nn.Linear(rec_dim, self.k_dim * 2), activation)  # Linear layer and activation
    ...
    def forward(self, xi, xpose, xf, xprox, state=None):
        """
        ...
        hid = torch.cat([enc_pts, xpose, xf, xprox], -1)

        rnn_hid = self.rec(hid, state)  # LSTM forward pass
        y_pose = self.decoder_pose(rnn_hid[0])  # Decode joint prediction
        y_force = self.decoder_force(rnn_hid[0])  # Decode joint prediction
        y_prox = self.decoder_prox(rnn_hid[0])  # Decode joint prediction
        ...
        return y_image, y_pose, y_force, y_prox, enc_pts, dec_pts, rnn_hid