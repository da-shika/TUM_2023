...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_left_vec = nn.Sequential(nn.Linear(rec_dim, vec_dim), activation)
        self.decoder_left_point = nn.Sequential(nn.Linear(rec_dim, self.k_dim * 2), activation)
        self.decoder_right_vec = nn.Sequential(nn.Linear(rec_dim, vec_dim), activation)
        self.decoder_right_point = nn.Sequential(nn.Linear(rec_dim, self.k_dim * 2), activation)
    ...
    def forward(self, xi, xpos, state=[None,None,None]):
        ...
        y_right_pose = self.decoder_right_vec(right_hid[0])
