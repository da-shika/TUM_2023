...
class OnlineSARNN:
    ...
    def reset(self):
        ...
        self.left_h_state_list, self.right_h_state_list, self.union_h_state_list = [], [], []
        self.left_c_state_list, self.right_c_state_list, self.union_c_state_list = [], [], []
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def run(self):
        ...
        while not rospy.is_shutdown() and self.steps < self.max_episode_steps:
            y_left_image, y_right_image, y_left_pose, y_right_pose, left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state = self.inference(state)
            ...
            self.append_params(y_left_image, y_right_image, y_left_pose, y_right_pose, left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def inference(self, state):
        ...
        with torch.no_grad():
            y_image, y_pose, enc_pts, dec_pts, state = self.model(x_image, x_pose, state)
        ...
        return y_left_image, y_right_image, y_left_pose, y_right_pose, left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state
    ...
    def append_params(self, y_left_image, y_right_image, y_left_pose, y_right_pose, left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state):
        ...
        self.left_h_state_list.append(tensor2numpy(state[0][0]))
        self.right_h_state_list.append(tensor2numpy(state[1][0]))
        self.union_h_state_list.append(tensor2numpy(state[2][0]))
        self.left_c_state_list.append(tensor2numpy(state[0][1]))
        self.right_c_state_list.append(tensor2numpy(state[1][1]))
        self.union_c_state_list.append(tensor2numpy(state[2][1]))
    ...
    def save(self):
        ...
        left_h_state = np.array(self.left_h_state_list)
        right_h_state = np.array(self.right_h_state_list)
        union_h_state = np.array(self.union_h_state_list)
        left_c_state = np.array(self.left_c_state_list)
        right_c_state = np.array(self.right_c_state_list)
        union_c_state = np.array(self.union_c_state_list)
        ...
        np.savez(save_dir +"/"+ datetime.datetime.today().strftime("%Y%m%d_%H%M_%S_") + user_input, 
                 ...
                 left_h_state=left_h_state, right_h_state=right_h_state, union_h_state=union_h_state,
                 left_c_state=left_c_state, right_c_state=right_c_state, union_c_state=union_c_state
                 )
