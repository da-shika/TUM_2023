...
class HLSTMLayer(nn.Module):
    def __init__(self, in_dim, rec_dim, union_dim):
        ...
        self.union_rnn = nn.LSTMCell(rec_dim * 4, union_dim)
        self.union_out = nn.Linear(union_dim, rec_dim * 4)

    def forward(self, xl, xr, state=[None, None, None]):
        left_hid = self.left_rnn(xl, state[0])
        right_hid = self.right_rnn(xr, state[1])

        union_input = torch.concat((left_hid[0], right_hid[0], left_hid[1], right_hid[1]), axis=-1)
        union_hid = self.union_rnn(union_input, state[2])
        union_out = self.union_out(union_hid[0])
        _union_left_h, _union_right_h, _union_left_c, _union_right_c = torch.split(union_out, self.rec_dim, dim=-1)

        union_left = [_union_left_h, _union_left_c]
        union_right = [_union_right_h, _union_right_c]
...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        vec_dim = pose_dim + force_dim + proximity_dim
    ...
    def forward(self, xi, xpos, xf, xp, state=[None,None,None]):
        """
        ...
        # concat
        xv_l = torch.cat((xpos[0], xf[0], xp[0]), -1)
        xv_r = torch.cat((xpos[1], xf[1], xp[1]), -1)
        ...
        split_points = [self.pose_dim, self.force_dim, self.proximity_dim]
        y_left_pose, y_left_force, y_left_proximity = torch.split(y_left_vec, split_points, dim=1)
        y_right_pose, y_right_force, y_right_proximity = torch.split(y_right_vec, split_points, dim=1)
