...
class fullBPTTtrainer:
    ...
    def __init__(self, model, optimizer, loss_weights=[1.0, 1.0], device="cpu", img_decay_end=200, force_decay_end=1000):
        self.device = device
        self.optimizer = optimizer
        self.loss_weights = loss_weights
        self.image_scheduler = LossScheduler(decay_end=img_decay_end, curve_name="s")
        self.force_scheduler = LossScheduler(decay_end=force_decay_end, curve_name="s")
    ...
    def process_epoch(self, data, epoch, image_stop_epoch, training=True):
        if not training:
            self.model.eval()
        else:
            self.model.train()
        ...
        total_loss = 0.0
        for n_batch, (
            (x_left_img, x_right_img, x_left_pose, x_right_pose, x_left_force, x_right_force, x_left_proximity, x_right_proximity),
            (y_left_img, y_right_img, y_left_pose, y_right_pose, y_left_force, y_right_force, y_left_proximity, y_right_proximity)) in enumerate(data):
            ...
            for t in range(T - 1):
                ...
                _yi_hat, _ypose_hat, _yf_hat, _yp_hat, enc_ij, dec_ij, state = self.model(x_img, x_pose, x_force, x_proximity, epoch, image_stop_epoch, state)
            ...
            left_pt_loss = nn.MSELoss()(left_enc_pts, left_dec_pts) * self.image_scheduler(self.loss_weights[2])
            right_pt_loss = nn.MSELoss()(right_enc_pts, right_dec_pts) * self.image_scheduler(self.loss_weights[2])
            
            left_force_loss =nn.MSELoss()(y_left_force_hat, y_left_force[:, 1:]) * self.force_scheduler(self.loss_weights[3])
            right_force_loss =nn.MSELoss()(y_right_force_hat, y_right_force[:, 1:]) * self.force_scheduler(self.loss_weights[3])
            left_proximity_loss =nn.MSELoss()(y_left_proximity_hat, y_left_proximity[:, 1:]) * self.force_scheduler(self.loss_weights[3])
            right_proximity_loss =nn.MSELoss()(y_right_proximity_hat, y_right_proximity[:, 1:]) * self.force_scheduler(self.loss_weights[3])
