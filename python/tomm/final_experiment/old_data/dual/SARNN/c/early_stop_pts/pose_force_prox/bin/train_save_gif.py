...
parser.add_argument("--image_decay_end", type=int, default=1000)
parser.add_argument("--force_decay_end", type=int, default=1000)
parser.add_argument("--image_stop_epoch", type=int, default=400)
...
image_stop_epoch = args.image_stop_epoch
...
with tqdm(range(args.epoch)) as pbar_epoch:
    for epoch in pbar_epoch:
        ...
        model.to(device)
        # train and test
        train_loss = trainer.process_epoch(train_loader, epoch, image_stop_epoch)
        with torch.no_grad():
            test_loss = trainer.process_epoch(test_loader, epoch, image_stop_epoch, training=False)
        writer.add_scalar("Loss/train_loss", train_loss, epoch)
        writer.add_scalar("Loss/test_loss", test_loss, epoch)  

        if epoch % 20 == 0 and epoch < 201:
            with torch.no_grad():
                model.to("cpu")
                model.eval()
                ...
                for step in range(len(test_dataset[0][0][0])):
                    ...
                    y_image, _, _, _, enc_pts, dec_pts, state = model(image, pose, force, prox, epoch, image_stop_epoch, state)
                ...
                write_gif(log_dir_path, "left_image_{}.gif".format(epoch), left_image_list, "Left Image")
                write_gif(log_dir_path, "right_image_{}.gif".format(epoch), right_image_list, "Right Image")
