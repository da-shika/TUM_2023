...
class MTRNNCell(nn.Module):
    ...
    def __init__(self,
        ...
        def _make_one_side_fast_layer():
            ...
            return pose_f2f, pose_f2s, enc_f2f, enc_f2s
        ...
        def _make_one_side_slow_layer():
            ...
            return s2s, pose_s2f, enc_s2f, s2u, u2s
        
        # Union context layer
        self.u2u = nn.Linear(union_dim, union_dim, bias=False)

        # Left side
        self.left_pose_i2f, self.left_enc_i2f= _make_one_side_input_layer()
        self.left_pose_f2f, self.left_pose_f2s, self.left_enc_f2f, self.left_enc_f2s = _make_one_side_fast_layer()
        self.left_s2s, self.left_pose_s2f, self.left_enc_s2f, self.left_s2u, self.left_u2s = _make_one_side_slow_layer()

        # Right side
        self.right_pose_i2f, self.right_enc_i2f = _make_one_side_input_layer()
        self.right_pose_f2f, self.right_pose_f2s, self.right_enc_f2f, self.right_enc_f2s = _make_one_side_fast_layer()
        self.right_s2s, self.right_pose_s2f, self.right_enc_s2f, self.right_s2u, self.right_u2s = _make_one_side_slow_layer()


    def forward(self, xl_pose, xl_enc, xr_pose, xr_enc, state=[None, None, None], pb=None):
        ...
        if state is not None and all(s is not None for s in state):
            prev_left_pose_h_fast, prev_left_enc_h_fast, prev_left_h_slow, prev_left_pose_u_fast, prev_left_enc_u_fast, prev_left_u_slow = state[0]
            prev_right_pose_h_fast, prev_right_enc_h_fast, prev_right_h_slow, prev_right_pose_u_fast, prev_right_enc_u_fast, prev_right_u_slow = state[1]
            ...
            def _make_zero_prev():
                ...
                return pose_fast, enc_fast, slow
            
            # context state
            prev_h_union = torch.zeros(batch_size, self.union_dim).to(device)
            prev_left_pose_h_fast, prev_left_enc_h_fast, prev_left_h_slow = _make_zero_prev()
            prev_right_pose_h_fast, prev_right_enc_h_fast, prev_right_h_slow = _make_zero_prev()

            # internal state
            prev_u_union = torch.zeros(batch_size, self.union_dim).to(device)
            prev_left_pose_u_fast, prev_left_enc_u_fast, prev_left_u_slow = _make_zero_prev()
            prev_right_pose_u_fast, prev_right_enc_u_fast, prev_right_u_slow = _make_zero_prev()
        ...
        def _update_u(prev_pose_u_fast, pose_i2f, x_pose, pose_f2f, prev_pose_h_fast, pose_s2f, prev_h_slow,\
                      ...
                      pose_f2s, enc_f2s, s2s, u2s, prev_u_slow):
            ...
            _input_slow = pose_f2s(prev_pose_h_fast) + enc_f2s(prev_enc_h_fast) + s2s(prev_h_slow) + u2s(prev_h_union)
            if pb is not None:
                _input_slow += pb
            
            new_u_slow = (1.0 - 1.0/self.slow_tau) * prev_u_slow + 1.0 / self.slow_tau * _input_slow
            return new_pose_u_fast, new_enc_u_fast, new_u_slow
        
        def _update_h(pose_u, enc_u, slow_u):
            new_pose_h_fast = self.activation(pose_u)
            new_enc_h_fast = self.activation(enc_u)
            new_h_slow = self.activation(slow_u)
            return new_pose_h_fast, new_enc_h_fast, new_h_slow, 
        
        # left MTRNN
        new_left_pose_u_fast, new_left_enc_u_fast, new_left_u_slow = \
            _update_u(prev_left_pose_u_fast, self.left_pose_i2f, xl_pose, self.left_pose_f2f, prev_left_pose_h_fast, self.left_pose_s2f, prev_left_h_slow,\
                    ...
                    self.left_pose_f2s, self.left_enc_f2s, self.left_s2s, self.left_u2s, prev_left_u_slow)
        
        new_left_pose_h_fast, new_left_enc_h_fast, new_left_h_slow = \
          _update_h(new_left_pose_u_fast, new_left_enc_u_fast, new_left_u_slow)
        

        # right MTRNN
        new_right_pose_u_fast, new_right_enc_u_fast, new_right_u_slow =\
            _update_u(prev_right_pose_u_fast, self.right_pose_i2f, xr_pose, self.right_pose_f2f, prev_right_pose_h_fast, self.right_pose_s2f, prev_right_h_slow,\
                      ...
                      self.right_pose_f2s, self.right_enc_f2s, self.right_s2s, self.right_u2s, prev_right_u_slow)
        
        new_right_pose_h_fast, new_right_enc_h_fast, new_right_h_slow = _update_h(new_right_pose_u_fast, new_right_enc_u_fast, new_right_u_slow)
        ...
        left_hid = [new_left_pose_h_fast, new_left_enc_h_fast, new_left_h_slow,
                    new_left_pose_u_fast, new_left_enc_u_fast, new_left_u_slow]
        right_hid = [new_right_pose_h_fast, new_right_enc_h_fast, new_right_h_slow, 
                     new_right_pose_u_fast, new_right_enc_u_fast, new_right_u_slow]
...
class MTRNN(nn.Module):
    ...
    def forward(self, xi, xpos, state=[None,None,None]):
        ...
        # rnn
        left_hid, right_hid, state = self.rec(xpos[0], left_enc_pts, xpos[1], right_enc_pts, state)
        ...
        return [y_left_image, y_right_image], [y_left_pose, y_right_pose], [left_enc_pts, right_enc_pts], [left_dec_pts, right_dec_pts], state