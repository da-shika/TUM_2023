...
left_image, left_pose, left_force, left_proximity = load_data(train='test', data_type='left')
right_image, right_pose, right_force, right_proximity = load_data(train='test', data_type='right')
...
# Inference
img_size = 64
image_list, pose_list, force_list, proximity_list = [], [], [], []
left_enc_pts_list, left_dec_pts_list = [], []
right_enc_pts_list, right_dec_pts_list = [], []
left_att_enc_list, left_att_force_list, left_att_prox_list = [], [], []
right_att_enc_list, right_att_force_list, right_att_prox_list = [], [], []
...
for loop_ct in range(nloop):
    ...
    force_t = [torch.Tensor(left_force[loop_ct : loop_ct + 1]),
            torch.Tensor(right_force[loop_ct : loop_ct + 1]),]
    ...
    # predict rnn
    y_image, y_pose, y_force, y_proximity, ect_pts, dec_pts, state, modality_att = model(img_t, pose_t, force_t, proximity_t, state)
    ...
    left_att_enc, left_att_force, left_att_prox = torch.split(modality_att[0], 1, dim=-1)
    right_att_enc, right_att_force, right_att_prox = torch.split(modality_att[1], 1, dim=-1)
    ...
    left_att_enc_list.append(tensor2numpy(left_att_enc))
    left_att_force_list.append(tensor2numpy(left_att_force))
    left_att_prox_list.append(tensor2numpy(left_att_prox))
    right_att_enc_list.append(tensor2numpy(right_att_enc))
    right_att_force_list.append(tensor2numpy(right_att_force))
    right_att_prox_list.append(tensor2numpy(right_att_prox))
...
left_att_enc = np.array(left_att_enc_list)
left_att_force = np.array(left_att_force_list)
left_att_prox = np.array(left_att_prox_list)
right_att_enc = np.array(right_att_enc_list)
right_att_force = np.array(right_att_force_list)
right_att_prox = np.array(right_att_prox_list)
...
# plot images
T = len(left_image)
fig, ax = plt.subplots(2, 5, figsize=(25, 10), dpi=60)
...
def anim_update(i):
    ...
    def plot_attention(att_enc, att_force, att_prox, ax_area, ylim_range, side):
        ...
        att_enc[:,0] = att_enc[:,0]/(att_sum[:,0]+0.01)
        att_force[:,0] = att_force[:,0]/(att_sum[:,0]+0.01)
        att_prox[:,0] = att_prox[:,0]/(att_sum[:,0]+0.01)
        ax_area.set_ylim(ylim_range)
        ax_area.set_xlim(0, T)
        ax_area.plot(att_enc.reshape(T,-1)[1:], linestyle="dashed", c="k")
        ax_area.plot(att_force.reshape(T,-1)[1:], linestyle="dashed", c="k")
        ax_area.plot(att_prox.reshape(T,-1)[1:], linestyle="dashed", c="k")
        ax_area.plot(np.arange(i + 1), att_enc.reshape(T,-1)[: i + 1])      # blue
        ax_area.plot(np.arange(i + 1), att_force.reshape(T,-1)[: i + 1])    # orange
        ax_area.plot(np.arange(i + 1), att_prox.reshape(T,-1)[: i + 1])     # green
        ax_area.set_xlabel("Step")
        ax_area.set_title(side + " modality attention")

    plot_data(left_pose, left_pose_bounds, right_pose, right_pose_bounds, pred_pose, \
              ax[1,0], (-1.0, 1.5), "Pose")
    plot_data(left_force, left_force_bounds, right_force, right_force_bounds, pred_force, \
              ax[1,1], (0.0, 4.0), "Force")
    plot_data(left_proximity, left_proximity_bounds, right_proximity, right_proximity_bounds, pred_proximity, \
              ax[1,2], (0.0, 1.0), "Proximity")
    plot_attention(left_att_enc, left_att_force, left_att_prox, ax[1,3], (-0.1, 1.1), "Left")
    plot_attention(right_att_enc, right_att_force, right_att_prox, ax[1,4], (-0.1, 1.1), "Right")
