...
class OnlineMTRNN:
    def __init__(self, data_dir, model_name, inference_rate=10, max_episode_steps=270):
        ...
        dir_name = "/home/genki/ros/workspaces/tomm_base_ws_local/bags/re_no_rotate/npz/dual" + "/"
    ...
    def run(self):
        ...
        for i in range(len(self.left_image)):
            ...
            y_left_image, y_right_image, y_left_pose, y_right_pose, y_left_force, y_right_force, y_left_proximity, y_right_proximity, \
                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, state, modality_att = self.inference(state, i)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def inference(self, state, i):
        ...
        x_force = [self.process_tactile(self.left_force[i], self.left_force_bounds),
                   self.process_tactile(self.right_force[i], self.right_force_bounds)]
        x_proximity = [self.process_tactile(self.left_proximity[i], self.left_proximity_bounds),
                        self.process_tactile(self.right_proximity[i], self.right_proximity_bounds)]
...
def main():
    ...
    MTRNN_motion = OnlineMTRNN(
        ...
        model_name="20231129_0207_39", 
        inference_rate=10,
        max_episode_steps=10000)
    MTRNN_motion.run()

    rospy.loginfo("online_MTRNN_node shutdown")
