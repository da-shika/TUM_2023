...
left_att_enc, left_att_prox = np.split(modality_att[0], 2, axis=1)
right_att_enc, right_att_prox = np.split(modality_att[1], 2, axis=1)
...
def anim_update(i):
    ...
    def plot_attention(att_enc, att_prox, ax_area, ylim_range, side):
        ...
        ax_area.plot(np.arange(i + 1), att_prox/att_sum.reshape(T,-1)[: i + 1])     # orange
        ax_area.set_xlabel("Step")
    ...
    plot_data(prev_left_prox, real_left_prox, ax[1,2], (0.0, 1.0), "Left prox")
    plot_data(prev_right_prox, real_right_prox, ax[1,3], (0.0, 1.0), "Right prox")
    plot_attention(left_att_enc, left_att_prox, ax[1,4], (0.0, 1.0), "Left")
    plot_attention(right_att_enc, right_att_prox, ax[1,5], (0.0, 1.0), "Right")
...
#ani.save(save_dir + "/MTRNN_{}.gif".format(base_name), writer="imagemagick")
#ani.save(save_dir + "/MTRNN_{}.gif".format(base_name), writer="ffmpeg")