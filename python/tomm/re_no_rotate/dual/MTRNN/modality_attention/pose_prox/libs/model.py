...
class MTRNNCell(nn.Module):
    ...
    def __init__(self,
        ...
        def _make_one_side_input_layer():
            ...
            return pose_i2f, enc_i2f, prox_i2f
        ...
        def _make_one_side_fast_layer():
            ...
            return pose_f2f, pose_f2s, enc_f2f, enc_f2s, prox_f2f, prox_f2s
        ...
        def _make_one_side_slow_layer():
            ...
            return s2s, pose_s2f, enc_s2f, prox_s2f, s2u, u2s
        
        # Union context layer
        self.u2u = nn.Linear(union_dim, union_dim, bias=False)

        # Left side
        self.left_pose_i2f, self.left_enc_i2f, self.left_prox_i2f = _make_one_side_input_layer()
        self.left_pose_f2f, self.left_pose_f2s, self.left_enc_f2f, self.left_enc_f2s, self.left_prox_f2f, self.left_prox_f2s = _make_one_side_fast_layer()
        self.left_s2s, self.left_pose_s2f, self.left_enc_s2f, self.left_prox_s2f, self.left_s2u, self.left_u2s = _make_one_side_slow_layer()

        # Right side
        self.right_pose_i2f, self.right_enc_i2f, self.right_prox_i2f = _make_one_side_input_layer()
        self.right_pose_f2f, self.right_pose_f2s, self.right_enc_f2f, self.right_enc_f2s, self.right_prox_f2f, self.right_prox_f2s = _make_one_side_fast_layer()
        self.right_s2s, self.right_pose_s2f, self.right_enc_s2f, self.right_prox_s2f, self.right_s2u, self.right_u2s = _make_one_side_slow_layer()


    def forward(self, xl_pose, xl_enc, xl_prox, xr_pose, xr_enc, xr_prox, state=[None, None, None], pb=None):
        ...
        if state is not None and all(s is not None for s in state):
            prev_left_pose_h_fast, prev_left_enc_h_fast, prev_left_prox_h_fast, prev_left_h_slow, \
                prev_left_pose_u_fast, prev_left_enc_u_fast, prev_left_prox_u_fast, prev_left_u_slow = state[0]
            prev_right_pose_h_fast, prev_right_enc_h_fast, prev_right_prox_h_fast, prev_right_h_slow, \
                prev_right_pose_u_fast, prev_right_enc_u_fast, prev_right_prox_u_fast, prev_right_u_slow = state[1]
            ...
            def _make_zero_prev():
                ...
                return pose_fast, enc_fast, prox_fast, slow
            
            # context state
            prev_h_union = torch.zeros(batch_size, self.union_dim).to(device)
            prev_left_pose_h_fast, prev_left_enc_h_fast, prev_left_prox_h_fast, prev_left_h_slow = _make_zero_prev()
            prev_right_pose_h_fast, prev_right_enc_h_fast, prev_right_prox_h_fast, prev_right_h_slow = _make_zero_prev()

            # internal state
            prev_u_union = torch.zeros(batch_size, self.union_dim).to(device)
            prev_left_pose_u_fast, prev_left_enc_u_fast, prev_left_prox_u_fast, prev_left_u_slow = _make_zero_prev()
            prev_right_pose_u_fast, prev_right_enc_u_fast, prev_right_prox_u_fast, prev_right_u_slow = _make_zero_prev()
        ...
        def _update_u(prev_pose_u_fast, pose_i2f, x_pose, pose_f2f, prev_pose_h_fast, pose_s2f, prev_h_slow,\
            ...
            _att_h_enc, _att_h_prox = torch.split(_att_h, 1, dim=-1)
            att_h_enc = _att_h_enc/(_att_h_enc + _att_h_prox + self.epsilon)
            att_h_prox = _att_h_prox/(_att_h_enc + _att_h_prox + self.epsilon)

            _input_slow = pose_f2s(prev_pose_h_fast) + enc_f2s(prev_enc_h_fast*att_h_enc) + \
            ...
            return new_pose_u_fast, new_enc_u_fast, new_prox_u_fast, new_u_slow, _att_h
        
        def _update_h(pose_u, enc_u, prox_u, slow_u):
            ...
            return new_pose_h_fast, new_enc_h_fast, new_prox_h_fast, new_h_slow, 
        
        # left MTRNN
        new_left_pose_u_fast, new_left_enc_u_fast, new_left_prox_u_fast, new_left_u_slow, left_attention = \
        ...
        new_left_pose_h_fast, new_left_enc_h_fast, new_left_prox_h_fast, new_left_h_slow = \
          _update_h(new_left_pose_u_fast, new_left_enc_u_fast, new_left_prox_u_fast, new_left_u_slow)
        

        # right MTRNN
        new_right_pose_u_fast, new_right_enc_u_fast, new_right_prox_u_fast, new_right_u_slow, right_attention =\
        ...
        new_right_pose_h_fast, new_right_enc_h_fast, new_right_prox_h_fast, new_right_h_slow = \
          _update_h(new_right_pose_u_fast, new_right_enc_u_fast, new_right_prox_u_fast, new_right_u_slow)
        ...
        left_hid = [new_left_pose_h_fast, new_left_enc_h_fast, new_left_prox_h_fast, new_left_h_slow,
                    new_left_pose_u_fast, new_left_enc_u_fast, new_left_prox_u_fast, new_left_u_slow]
        right_hid = [new_right_pose_h_fast, new_right_enc_h_fast, new_right_prox_h_fast, new_right_h_slow, 
...
class MTRNN(nn.Module):
    ...
    def forward(self, xi, xpos, xprox, state=[None,None,None]):
        """
        ...
        # rnn
        left_hid, right_hid, state, modality_attention = self.rec(xpos[0], left_enc_pts, xprox[0], xpos[1], right_enc_pts, xprox[1], state)

        # vec predict
        y_left_pose = self.decoder_left_pose(left_hid[0])
        left_dec_pts = self.decoder_left_point(left_hid[1])
        y_left_prox = self.decoder_left_prox(left_hid[2])

        y_right_pose = self.decoder_right_pose(right_hid[0])
        right_dec_pts = self.decoder_right_point(right_hid[1])
        y_right_prox = self.decoder_right_prox(right_hid[2])
