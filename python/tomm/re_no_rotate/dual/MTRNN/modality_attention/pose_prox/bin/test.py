...
left_image, left_pose, left_proximity = load_data(train='test', data_type='left')
right_image, right_pose, right_proximity = load_data(train='test', data_type='right')
...
# Inference
img_size = 64
image_list, pose_list, proximity_list = [], [], []
left_enc_pts_list, left_dec_pts_list = [], []
right_enc_pts_list, right_dec_pts_list = [], []
left_att_enc_list, left_att_prox_list = [], []
right_att_enc_list, right_att_prox_list = [], []
...
for loop_ct in range(nloop):
    ...
    # predict rnn
    y_image, y_pose, y_proximity, ect_pts, dec_pts, state, modality_att = model(img_t, pose_t,  proximity_t, state)
    ...
    left_att_enc, left_att_prox = torch.split(modality_att[0], 1, dim=-1)
    right_att_enc, right_att_prox = torch.split(modality_att[1], 1, dim=-1)
...
def anim_update(i):
    ...
    def plot_attention(att_enc, att_prox, ax_area, ylim_range, side):
        ax_area.set_ylim(ylim_range)
        ax_area.set_xlim(0, T)
        ...
        ax_area.plot(np.arange(i + 1), att_prox.reshape(T,-1)[: i + 1])     # orange
        ax_area.set_xlabel("Step")
    ...
    plot_attention(left_att_enc, left_att_prox, ax[1,2], (0.0, 1.0), "Left")
    plot_attention(right_att_enc, right_att_prox, ax[1,3], (0.0, 1.0), "Right")
