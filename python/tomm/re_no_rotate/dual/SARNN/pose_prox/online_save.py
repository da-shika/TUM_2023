...
#-----------------------------------------------------------------------------------------------------------------------------------------
    def run(self):
        ...
        while not rospy.is_shutdown() and self.steps < self.max_episode_steps:
            ...
            self.append_params(y_left_image, y_right_image, y_left_pose, y_right_pose, y_left_proximity, y_right_proximity, \
                                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    def append_params(self, y_left_image, y_right_image, y_left_pose, y_right_pose, y_left_proximity, y_right_proximity, \
                        left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts):
    ...
    def save(self):
        ...
        np.savez(save_dir +"/"+ datetime.datetime.today().strftime("%Y%m%d_%H%M_%S"), 
                 ...
                 prev_left_pose=prev_left_pose, prev_right_pose=prev_right_pose, prev_left_prox=prev_left_prox, prev_right_prox=prev_right_prox,
                 real_left_pose=real_left_pose, real_right_pose=real_right_pose, real_left_prox=real_left_prox, real_right_prox=real_right_prox
                 )
