...
class PR2ViveControll:
    def __init__(self):
        ...
        self.left_target_sub = rospy.Subscriber("/pr2_left_target_pos", Float32MultiArray, self.leftcallback)
        self.right_target_sub = rospy.Subscriber("/pr2_right_target_pos", Float32MultiArray, self.rightcallback)
    ...
    def computeIK(self, target_position, side):
        # trajectory
        target_pose = trajectory(target_position)
        # compute ik
    ...
    def move_both_arms(self):
        if self.cmd == "ON":
            if self.left_target_position is not None and self.right_target_position is not None:
                ...
                left_req = self.computeIK(self.left_target_position, "left")
                ...
                right_req = self.computeIK(self.right_target_position, "right")
