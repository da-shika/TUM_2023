...
class PR2ViveControll:
    ...
    def config(self):
        ...
        self.left_wrist_limit_x = [0.750233, 0.201725]
        self.left_wrist_limit_y = [0.774648, -0.263164]
        self.right_wrist_limit_x = [0.767309, 0.358366]
        self.right_wrist_limit_y = [0.248343, -0.688685]
...
#----------------------------------------------------------------------------------------------
        ...
        while not rospy.is_shutdown():
            if self.cmd == "ON":
                if self.left_target_position is not None and self.right_target_position is not None:
                    ...
                    if left_res.error_code.val != 1:
                        rospy.logwarn_throttle(0.5, "left: {}".format(left_res.error_code.val))
                    ...
                    if right_res.error_code.val != 1:
                        rospy.logwarn_throttle(0.5, "right: {}".format(right_res.error_code.val))
    ...
    def set_limit(self):
        if (self.left_target_position[0]-0.1) > self.left_wrist_limit_x[0]:
            self.left_target_position[0] = self.left_wrist_limit_x[0]
            rospy.logwarn_throttle(0.5, "Left arm is out range")
        elif self.left_target_position[0] < self.left_wrist_limit_x[1]:
            self.left_target_position[0] = self.left_wrist_limit_x[1]
            rospy.logwarn_throttle(0.5, "Left arm is out range")

        if (self.right_target_position[0]-0.1) > self.right_wrist_limit_x[0]:
            self.right_target_position[0] = self.right_wrist_limit_x[0]
            rospy.logwarn_throttle(0.5, "Right arm is out range")
        elif self.left_target_position[0] < self.left_wrist_limit_x[1]:
            self.right_target_position[0] = self.right_wrist_limit_x[1]
            rospy.logwarn_throttle(0.5, "Right arm is out range")
